...
Ninth, `kolam-rangoli-mandala-indian-art-puzzle-game` on 13 Sep 2026 — **the first post of a
RevenueCat Shipaton 2026 series** about `products/curvved` — the game renamed **ChitraYatra** on
17 Sep 2026, and the post was republished on 19 Sep with the new name, a note saying so, and a
link to `/products/chitrayatra` (its own bundle; the Glass
mechanic and the art-sourcing decision live there). It covers the Indian art traditions
chosen for the game: kolam, rangoli and mandala generated by the game's own code; Gond,
Madhubani and Warli held for commissioned artists; deities and yantras excluded. Its
figures are first-party: `tool/glass_check.dart` counts, and a seeded sampling run
against `KolamSpec` (the gcd rule on 81 of 81 grids, and the square-grid symmetry counts).
Its five inline SVGs are **emitted by the game's own generators** (`KolamSpec`,
`OrnamentSpec`) through a scratch Dart script, drawn in `currentColor` with fill-opacity
standing in for the palette. The cost is page weight: ~55 KB of path data, and the live
