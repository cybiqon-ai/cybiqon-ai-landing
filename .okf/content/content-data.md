...
| File | Feeds |
|---|---|
| `data/products.ts` | `/products`, the three category pages and all eight product pages — types, `CATEGORIES`, `PRODUCTS`, `getProduct`, `productsIn`, `activeCategories` |
...
| `data/legal/{llmbytes,meflow,vitaloop,lumina,orbitone,chitrayatra,mapwit,page-arcade}.ts` | privacy and terms, as a `Block` discriminated union (prose / checklist / deflist / table / contact) rather than MDX or raw HTML |
